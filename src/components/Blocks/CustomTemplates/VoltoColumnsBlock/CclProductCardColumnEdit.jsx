...
import { CardBlockSchema } from './columnSchema';
...
CclProductCardColumnEdit -> passes EditBlockWrapper into
  -> BlocksForm -> which passes (with EditBlock) into
    -> DragDropList -> which renders them all
*/
class CclProductCardColumnEdit extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      multiSelected: [],
  ...
  render() {
    ...
    const ColumnSchema = CardBlockSchema(this.props.intl);
    ...
    return (
      <div
        role="presentation"
        className="columns-block product-columns-block"
        onKeyDown={(e) => {
          this.handleKeyDown(
            e,
            this.props.index,
          ...
          <>
            ...
            <Grid columns={gridSize} className="column-grid" stackable>
              {columnList.map(([colId, column], index) => (
                ...
                >
                  {colId === columnList[0][0] && (
                    <span className="product-card-column-header">
                      {columnList[0][1]?.settings && (
                        <>
